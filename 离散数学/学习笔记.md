...
<table width="800" border="2"><tr align="center"><th width="300" bgcolor="#ddAedd">定理</th><th width="500" bgcolor="#ddAedd">公式</th></tr><tr><td><font size=4>量词否定等价式</font></td><td><font size=4>¬∀xQ(x)⇔∃x ¬Q(x)<br>¬∃xQ(x)⇔∀x ¬Q(x)</font></td></tr><tr align="center"><td><font size=4>量词辖域缩小或扩大等价式<br>（设B是不含X自由出现， 查A(x)为有x自由出现的任意公式）</font></td><td><font size=4>(∀x)(A(x)∧B)⇔(∀x)A(x)∧B<br>(∀x)(A(x)∨B)⇔(∀x)A(x)∨B<br>(∀x)(A(x)→B)⇔(∃x)A(x)→B<br>(∀x)(B→A(x))⇔B→(∀x)A(x)<br>(∃x)(A(x)∧B)⇔(∃x)A(x)∧B<br>(∃x)(A(x)∨B)⇔(∃x)A(x)∨B<br>(∃x)(A(x)→B)⇔(∀x)A(x)→B<br>(∃x)(B→A(x))⇔B→(∃x)A(x)</font></td></tr><tr align="center"><td><font size=4>量词分配律等价式</font></td><td><font size=4>(∀x)(A(x)∧B(x))⇔(∀x)A(x)∧(∀x)B(x)<br>(∃x)(A(x)∨B(x))⇔(∃x)A(x)∨(∃x)B(x)</font></td></tr></font></td></tr></table>
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTIzOTk2MTk4MywtMTIyMzE5NTE2MiwtMT

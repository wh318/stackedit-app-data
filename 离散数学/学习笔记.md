...
<table width="800" border="2"><tr align="center"><th width="200" bgcolor="#ddAedd">定理</th><th width="600" bgcolor="#ddAedd">公式</th></tr><tr align="center"><td>量词否定等价式</td><td>¬∀xQ(x)⇔∃x ¬Q(x)<br>¬∃xQ(x)⇔∀x ¬Q(x)</td></tr><tr align="center"><td>量词辖域缩小或扩大等价式<br>（设B是不含X自由出现， 查A(x)为有x自由出现的任意公式）</td><td>(∀x)(A(x)∧B)⇔(∀x)A(x)∧B<br>(∀x)(A(x)∨B)⇔(∀x)A(x)∨B<br>(∀x)(A(x)→B)⇔(∃x)A(x)→B<br>(∀x)(B→A(x))⇔B→(∀x)A(x)<br>(∃x)(A(x)∧B)⇔(∃x)A(x)∧B<br>(∃x)(A(x)∨B)⇔(∃x)A(x)∨B<br>(∃x)(A(x)→B)⇔(∀x)A(x)→B<br>(∃x)(B→A(x))⇔B→(∃x)A(x)</td></tr><tr align="center"><td>量词分配律等价式</td><td>(∀x)(A(x)∧B(x))⇔(∀x)A(x)∧(∀x)B(x)<br>(∃x)(A(x)∨B(x))⇔(∃x)A(x)∨(∃x)B(x)</td></tr></td></tr></table>
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTYxNTM5NDAwLC0xNzU2NTU2OTE0LC05OD
Y0NzEwOTgsLTE5MzY3NzM2MzYsMjAzODY1ODg2OSwyMDUwMzU0
NzUzLDYyMzk2MzYzNywxNjEzMjg4MTU2LC02NjUwMjQwMDBdfQ
==
-->
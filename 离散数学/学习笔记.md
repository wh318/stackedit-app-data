...
<table width="800" border="2"><tr align="center"><th width="300" bgcolor="#ddAedd">定理</th><th width="500" bgcolor="#ddAedd">公式</th></tr><tr align="center"><td><font size=4>量词否定等价式</font></td><td><font size=4>¬∀xQ(x)⇔∃x ¬Q(x)</td><td>¬∃xQ(x)⇔∀x ¬Q(x)</font></td></tr><tr align="center"><td><font size=4>量词辖域缩小或扩大等价式</td><td>（设B是不含X自由出现， 查A(x)为有x自由出现的任意公式）</font></td><td><font size=4>(∀x)(A(x)∧B)⇔(∀x)A(x)∧B</td><td>(∀x)(A(x)∨B)⇔(∀x)A(x)∨B</td><td>(∀x)(A(x)→B)⇔(∃x)A(x)→B</td><td>(∀x)(B→A(x))⇔B→(∀x)A(x)</td><td>(∃x)(A(x)∧B)⇔(∃x)A(x)∧B</td><td>(∃x)(A(x)∨B)⇔(∃x)A(x)∨B</td><td>(∃x)(A(x)→B)⇔(∀x)A(x)→B</td><td>(∃x)(B→A(x))⇔B→(∃x)A(x)</font></td></tr><tr align="center"><td><font size=4>量词分配律等价式</font></td><td><font size=4>(∀x)(A(x)∧B(x))⇔(∀x)A(x)∧(∀x)B(x)</td><td>(∃x)(A(x)∨B(x))⇔(∃x)A(x)∨(∃x)B(x)</font></td></tr></font></td></tr></table>

<table width="800" border="2">
<tr align="center"><th width="300" bgcolor="#ddAedd">定理</th><th width="500" bgcolor="#ddAedd">公式</th></tr>
<tr align="center"><td rowspan="2">量词否定等价式</td><td><font size="4">¬∀xQ(x)⇔∃x ¬Q(x)</font></td></tr>
<tr align="center"><td><font size="4">¬∃xQ(x)⇔∀x ¬Q(x)</font></td></tr>
<tr align="center"><td rowspan="8">量词辖域缩小或扩大等价式<br>（设B是不含X自由出现， 查A(x)为有x自由出现的任意公式）</td><td><font size="4">(∀x)(A(x)∧B)⇔(∀x)A(x)∧B</font></td></tr>
<tr align="center"><td><font size="4">(∀x)(A(x)∨B)⇔(∀x)A(x)∨B</font></td></tr>
<tr align="center"><td><font size="4">(∀x)(A(x)→B)⇔(∃x)A(x)→B</font></td></tr>
<tr align="center"><td><font size="4">(∀x)(B→A(x))⇔B→(∀x)A(x)</font></td></tr>
<tr align="center"><td><font size="4">(∃x)(A(x)∧B)⇔(∃x)A(x)∧B</font></td></tr>
<tr align="center"><td><font size="4">(∃x)(A(x)∨B)⇔(∃x)A(x)∨B</font></td></tr>
<tr align="center"><td><font size="4">(∃x)(A(x)→B)⇔(∀x)A(x)→B</font></td></tr>
<tr align="center"><td><font size="4">(∃x)(B→A(x))⇔B→(∃x)A(x)</font></td></tr>
<tr align="center"><td rowspan="2">量词分配律等价式</td><td><font size="4">(∀x)(A(x)∧B(x))⇔(∀x)A(x)∧(∀x)B(x)</font></td></tr>
<tr align="center"><td><font size="4">(∃x)(A(x)∨B(x))⇔(∃x)A(x)∨(∃x)B(x)</font></td></tr>
</table>
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTk1MjMxMTg5MiwxOTc2ODE5MTg4LC0xMj
IzMTk1MTYyLC0xNzU2NTU2OTE0LC05ODY0NzEwOTgsLTE5MzY3
NzM2MzYsMjAzODY1ODg2OSwyMDUwMzU0NzUzLDYyMzk2MzYzNy
wxNjEzMjg4MTU2LC02NjUwMjQwMDBdfQ==
-->